...
def create_reports(subfolder_reports):
    ...
    # Specify the general report file name
    general_report_file_name = os.path.join("reports", f'APLSTATS-GENERAL-REPORTE-{current_datetime}.log')

    # Create the directory if it doesn't exist
    os.makedirs(os.path.dirname(general_report_file_name), exist_ok=True)

    with open(general_report_file_name, 'w') as general_report:
        for simulation_name, execution_list in subfolder_reports.items():
            for execution_data in execution_list:
                if isinstance(execution_data, str):
                    # Skip strings (non-dictionary data)
                    continue

                summary_data = execution_data.get("summary", [])
                device_counts = execution_data.get("device_counts", {})
                execution_name = execution_data.get("execution_name", "Unknown")

                # Specify the report file name for each simulation execution
                report_file_name = os.path.join("reports", f'APLSTATS-REPORTE-{simulation_name}-{execution_name}-{current_datetime}.log')

                with open(report_file_name, 'w') as report:
                    report.write(f"\n=== Simulation: {simulation_name} - Execution: {execution_name} ===\n")
                ...
                print(f"Report file '{report_file_name}' created successfully.")
                general_report.write(f"General report for '{simulation_name} - {execution_name}': '{report_file_name}'\n")

    print(f"General report file '{general_report_file_name}' created successfully.")
"""

"""
THIS ONE WORKS BUT CREATE EXTRA FOLDER OF REPORTS
def create_reports(subfolder_reports):
    for subfolder_name, data in subfolder_reports.items():
        summary_data = data["summary"]
        device_counts = data["device_counts"]
        ...
        # Include the new subfolder level in the report file name
        subfolder_parts = subfolder_name.split(os.path.sep)
        subfolder_name_for_file = os.path.join(*subfolder_parts[-2:])  # Take the last two parts for the new subfolder level
        report_file_name = os.path.join("reports", f'APLSTATS-REPORTE-{subfolder_name_for_file}-{current_datetime}.log')
